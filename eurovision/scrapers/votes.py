...
class VotesScraper(BaseScraper):
    ...
    def scrape_votes(self, contest, table_data_attrib=None):
        ...
        # If we need tele or jury votes, they are usually in separate tables in the HTML
        if table_data_attrib:
            # Eurovision World uses class names like "scoreboard_table tele" or "scoreboard_table jury"
            # or uses a 'data-button' attribute to identify them.
            voting_grid = self.soup.find("table", {"class": "scoreboard_table", "data-button": table_data_attrib})
            
            # Fallback if the data-button attribute isn't directly on the table
            if not voting_grid:
                voting_grids = self.soup.find_all("table", {"class": "scoreboard_table"})
                # Typically: index 0 is Total, index 1 is Tele, index 2 is Jury
                if table_data_attrib == "tele" and len(voting_grids) > 1:
                    voting_grid = voting_grids[1]
                elif table_data_attrib == "jury" and len(voting_grids) > 2:
                    voting_grid = voting_grids[2]
                else:
                    voting_grid = voting_grids[0] if voting_grids else None
        else:
    ...
    def scrape_misc(self, contest):
        def get_items_for_contestant(contestant):
            ...
            html = self.get_html_via_flaresolverr(url)
            if not html:
                raise Exception("Failed to bypass Cloudflare for contestant page.")
